...
const Settings = () => {
  ...
  const settingArray = [
    ...
    { "key": "logo45", "label": "logo label3", "placeholder": "checkbox1", "type": "checkbox" },//file
        ...
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          <div className="flex flex-col gap-5.5 ">
            <form action="#">
              ...
              <div className="mb-4 grid p-6.5 grid-cols-3 gap-4">
  {text.map((item, index) => {
    if (item.type === "file") {
      return (
   ...
   // Add this line to handle other types of items if needed
  })}
                  {/* <img src={image_name} /> */}
                </div>
  ...
  {text.map((item, index) => {
    if (item.type === "text") {
      return (
        ...
        <button className="flex w-25 justify-center text-xs rounded-full bg-secondary py-2 px-3 font-medium text-white mb-4" onClick={handleEdit} >
          Update
        </button>
        </div>
        </div>
