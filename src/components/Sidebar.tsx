...
import imagelogo from "../images/logo/logo-dark.svg"
...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  const [userType, setUserTyoe] = useState(false);
  ...
  const [permission_rental, setPermissionRental] = useState(false);
      ...
      for (let i = 0; i < response.data.user_permissions.length; i++) {
        ...
        if ((response.data.user_permissions[i].Name == "product")) {
         ...
         if ((response.data.user_permissions[i].Name == "rental_property")) {
          setPermissionRental(response.data.user_permissions[i].Value)
         }
